#include "../src/fixedpoint_fft.h"
#include <gtest/gtest.h>
#include "etl/array.h"

// Demonstrate some basic assertions.
TEST(eFFT_Fixed_Unknown, SineWave) {
    // 697Hz sine wave sampled at 8kHz for 512 samples (produced by Audacity)
    unsigned char raw_697hz[] = {
    ...
    constexpr int N = 256;
    etl::array<int16_t, N> samples;
    std::memcpy(samples.data(), raw_697hz, sizeof(raw_697hz));
    EXPECT_EQ(samples[1], 0x354d);
    EXPECT_EQ(samples[255], 0x642e);
    
    
    FixedPointFFT<N, int16_t, uint16_t, WindowType::BlackmanHarris> fft;
    etl::array<uint16_t, (N / 2) + 1> magnitudes; 
    
    fft.magnitudes(samples, magnitudes);

    printf("FFT abs output ):\n");
    for (size_t i = 0; i < 256; ++i) {
        printf("[%03d] %d\n", i, magnitudes[i]);
    }
    printf("\n");
    //EXPECT_EQ(out[0].real, 205407); // DC component should be near zero for AC-coupled input
